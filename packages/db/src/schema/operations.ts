...
export const depositRoutes = pgTable(
  'deposit_routes',
  {
    ...
    routeKind: text('route_kind').$type<'address_route' | 'solana_program_pay'>().notNull().default('address_route'),
...
export const fundingSubmissionAttempts = pgTable(
  'funding_submission_attempt',
  {
    ...
    chain: text('chain').$type<'base' | 'solana'>().notNull(),
    txHash: text('tx_hash').notNull(),
    status: text('status').$type<'submitted' | 'confirmed' | 'failed'>().notNull().default('submitted'),
...
export const outboxEvents = pgTable(
  'outbox_event',
  {
    ...
    status: text('status').$type<'pending' | 'processing' | 'processed' | 'dead_letter'>().notNull().default('pending'),
...
export const payoutInstructions = pgTable(
  'payout_instruction',
  {
    ...
    transferId: text('transfer_id')
      .notNull()
      .references(() => transfers.transferId),
    method: text('method').$type<'bank'>().notNull(),
