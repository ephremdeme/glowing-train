...
export function mapToUiStatus(
  ...
  _payoutStatus: string | null,
  pendingFundingSubmission?: { txHash: string; submittedAt: string } | null
): UiTransferStatus {
  if (backendStatus === 'TRANSFER_CREATED') return 'CREATED';
  if (backendStatus === 'AWAITING_FUNDING') return pendingFundingSubmission ? 'CONFIRMING' : 'AWAITING_DEPOSIT';
  if (backendStatus === 'FUNDING_CONFIRMED') return 'SETTLED';
